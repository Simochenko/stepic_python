...
for i in numbers1:
    ...
    for j in numbers2:
        if j.lower() in i.lower():
            count += 1
        if count == len(numbers2):
            numbers3.append(numbers1[n])
print(numbers3)
#
#
# всего 4 цикла
# 1. Создаем пустой список№1. Кладём туда все строки.
# 2. Создаем пустой список№2. Кладем туда все поисковые запросы
# 3. Создаем пустой список№3, куда будем добавлять потом нужные строки со всеми поисковыми запросами.
# Создаем счетчик.
#
# для i в списке№1:
#     для j в списке№2:
#
# Проверяем есть ли  j.lower() в i.lower(). Если есть, добавляем в счетчик +1.
# Если счетчик равен длине списка№2, в список№3 добавляем содержимое из списка№1. обнуляем счетчик.